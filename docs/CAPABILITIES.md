...
| `CF_ENABLE_AUDIT` | Pro 平替 | seafevents `Activity` 事件表 | 管理端只读查询、筛选、CSV 导出与页面；标签变更审计侧车 `cf_audit_event` | **已完成（Hub 侧）**；过滤契约测试通过，容器门禁 `cap audit` 全绿（2026-08-20，镜像含 dev 全量代码）：目录创建/重命名、文件上传、移动、删除、恢复查询，按类型/操作筛选，`source`（api/commit）筛选，标签增删/改名 before/after，CSV 导出，管理员 UI。P2-08 已补齐：查询/导出支持按时间/操作人/类型/对象/来源/结果/路径筛选，`source`/`result`/`before`/`after` 作为一等字段返回。不覆盖读取/下载访问日志、合规审计与长期留存（决策不追求） |
